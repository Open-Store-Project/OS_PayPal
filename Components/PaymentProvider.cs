...
namespace OS_PayPal
{
    public class OSPayPalPaymentProvider : Nevoweb.DNN.NBrightBuy.Components.Interfaces.PaymentsInterface
    {
        ...
        public override string GetTemplate(NBrightInfo cartInfo)
        {
            ...
            templ = NBrightBuyUtils.RazorTemplRender(templateName, 0, "", info, "/DesktopModules/NBright/OS_PayPal", "config", Utils.GetCurrentCulture(), passSettings);
        ...
        private string GetReturnTemplate(OrderData orderData, bool paymentok, string paymenterror)
        {
            ...
            if (paymentok)
            {
                ...
                templ = NBrightBuyUtils.RazorTemplRender(displaytemplate, 0, "", info, "/DesktopModules/NBright/OS_PayPal", "config", Utils.GetCurrentCulture(), passSettings);
            }
            else
            {
